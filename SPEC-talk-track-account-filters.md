...
## Record type mapping (inferred, unconfirmed)

uDab's code names only two record types (the ID constants above). The others exist only as raw `RecordTypeId` values. Inferred from account-name suffixes in the dev DB (2026-08-17 sync):

| RecordTypeId | Accounts | Evidence | Inferred label |
|---|---|---|---|
| `0124A000001BTyZQAW` | 2,920 | names end "- Parent" | **Parent** |
| `012A0000000kZbFIAU` | 1,043 | names end "- Inbound SDR", "- Social Boost" | **Inbound** |
| `0124w000001NbCpAAK` | 436 | names end "- Staffing" | **Talent**? (Abstrakt Talent Solutions = staffing arm) |
| `0124A000001Qc5kQAC` | 1,084 | internal-looking names ("Life at Abstrakt", "Cafe Piazza Orders") | internal/house accounts |

Official names could not be pulled from Salesforce (`RecordType` SOQL): the dev sandbox credentials are stale (`invalid_grant`). Confirm the mapping with the client or refresh sandbox creds before hard-coding anything.
...
1. **Record type options**: offer exactly Pipeline Client + Sapper Consulting like the Sequencer (implicitly excluding Parent/Inbound/Talent), or make Inbound/Talent selectable with the two as merely the default selection? Is "Talent" the record type on the "- Staffing" accounts?
2. **Default status selection**: Active only (Sequencer default), or Active + Implementing (what Reports targets today)?
